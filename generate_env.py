...
try:
    ...
    raise Exception(
        "Please install click and passlib with the following command: pip install passlib click"
    )
...
def comment_line(content, pattern):
    return [
        "#" + line if pattern in line and not line.strip().startswith("#") else line
        for line in content
    ]
...
def generate_env():
    ...
    mode = click.prompt(
        "Installation mode", type=click.Choice(["prod", "dev"]), default="prod"
    )
    ...
    if mode == "dev":
        ...
        dev_patterns = [
            ...
            "COMPOSE_FILE=docker-compose.essential.yml:docker-compose.traefik.yml:docker-compose.dev.yml",
        ]
        for pattern in dev_patterns:
            modified = uncomment_line(modified, pattern)

        if use_prebuilt:
            modified = uncomment_line(
                modified, "COMPOSE_PROFILES=pre-built-db,usershub"
            )
            ...
            use_external_db = click.confirm(
                "Use an external PostgreSQL database (outside docker-compose)?",
                default=False,
            )
            if use_external_db:
                default_postgres_host = (
                    get_default_from_sample("POSTGRES_HOST", content) or "localhost"
                )
                default_postgres_db = (
                    get_default_from_sample("POSTGRES_DB", content) or "geonature2db"
                )
                default_postgres_port = (
                    get_default_from_sample("POSTGRES_PORT", content) or "5432"
                )

                postgres_host = click.prompt(
                    "Postgres host", default=default_postgres_host
                )
                postgres_db = click.prompt(
                    "Postgres database name", default=default_postgres_db
                )
                postgres_port = click.prompt(
                    "Postgres port", default=default_postgres_port
                )
        ...
        defaults = {
            ...
            "POSTGRES_PORT_ON_HOST": get_default_from_sample(
                "POSTGRES_PORT_ON_HOST", content
            ),
            "POSTGRES_HOST": get_default_from_sample("POSTGRES_HOST", content)
            or "postgres",
            "POSTGRES_DB": get_default_from_sample("POSTGRES_DB", content)
            or "geonature2db",
            "POSTGRES_PORT": get_default_from_sample("POSTGRES_PORT", content)
            or "5432",
        }

        postgres_user = click.prompt(
            "Postgres username", default=defaults["POSTGRES_USER"]
        )
        ...
        if click.confirm("Generate a random postgres password?", default=False):
            ...
            postgres_password = click.prompt(
                "Postgres password",
                default=defaults["POSTGRES_PASSWORD"],
                hide_input=True,
            )

        if click.confirm(
            "Generate htpasswd hash for traefik (admin:<password>)?", default=False
        ):
            ...
            traefik_password = click.prompt(
                "Traefik password (htpasswd hash)",
                default=defaults["TRAEFIK_PASSWORD"],
                hide_input=False,
            )

        postgres_port_on_host = click.prompt(
            "Postgres port on host", default=defaults["POSTGRES_PORT_ON_HOST"]
        )
        ...
        if use_external_db:
            postgres_host = click.prompt(
                "Postgres host", default=defaults["POSTGRES_HOST"]
            )
            postgres_db = click.prompt(
                "Postgres database name", default=defaults["POSTGRES_DB"]
            )
            postgres_port = click.prompt(
                "Postgres port", default=defaults["POSTGRES_PORT"]
            )
        else:
