...
export COMPOSE_FILE=docker-compose.build.yml:$(docker compose config --environment | grep COMPOSE_FILE | awk -F= '{print $2}' || echo "docker-compose.yml")

docker compose build base-backend
docker compose build geonature-backend # requires base-backend
